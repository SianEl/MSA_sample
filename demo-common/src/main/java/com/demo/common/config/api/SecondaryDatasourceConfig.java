...
public class SecondaryDatasourceConfig {
    ...
    public DataSource secondaryDatasource(ApplicationConfig applicationConfig) {
        HikariConfig config = new HikariConfig();

        // DB 커넥션 관련 설정 - DB 기본 내용
        config.setDriverClassName(applicationConfig.getDatasource().getSecondaryDatasource().getDriverClassName()); //driver-class-name 설정
        config.setJdbcUrl(applicationConfig.getDatasource().getSecondaryDatasource().getJdbcUrl()); // jdbc-url 설정
        config.setUsername(applicationConfig.getDatasource().getSecondaryDatasource().getUsername()); // DB username 설정
        config.setPassword(applicationConfig.getDatasource().getSecondaryDatasource().getPassword()); // DB password 설정

        // DB 커넥션 관련 설정 - DB 연결 설정
        config.setAutoCommit(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().isAutoCommit()); // DB auto-commit 설정 기본 true
        config.setIdleTimeout(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().getIdleTimeout()); // pool에 일을 안하는 커넥션을 유지하는 시간. ms단위, minimumIdle 옵션이 maximumPoolSize보다 작게 설정되어 있을 때만 설정, 최소값은 10000ms(기본 600000)
        config.setMaxLifetime(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().getMaxLifetime()); // 커넥션 풀에서 살아있을 수 있는 커넥션의 최대 수명시간, 사용중이지 않은 커넥션만 제거, 풀 전체가 아닌 커넥션 별로 적용-(이유: 풀에서 대량으로 커넥션들이 제거되는 것을 방지)
        /*
        ...
        config.setMinimumIdle(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().getMinimumIdle()); // 아무런 일을 하지 않아도 적어도 유지해야 하는 pool 수, 기본값 : maximumPoolSize와 동일 -> 최적의 성능과 응답성을 요구한다면 이 값은 설정하지 않는게 좋다
        config.setMaximumPoolSize(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().getMaximumPoolSize()); // 최대 풀 사이즈
        config.setReadOnly(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().isReadOnly()); // pool에서 커넥션 획득할 때 read-only모드로 가져온다. 몇몇 DB는 read-only 모드를 지원하지 않음음
        config.setConnectionTimeout(applicationConfig.getDatasource().getSecondaryDatasource().getHikari().getConnectionTimeout()); // pool에서 커넥션을 얻어오기전까지 기다리는 최대 시간, 허용가능한 wait time을 초과하면 SQLException을 던진다. ms단위, 최소 설정시간-250ms, 기본시간-30000ms=30s
    ...
    public TransactionInterceptor txAdvice(@Qualifier("secondaryTransactionManager")TransactionManager transactionManager) {
    ...
    public DefaultPointcutAdvisor txAdviceAdvisor(@Qualifier("secondaryTransactionInterceptor") TransactionInterceptor transactionInterceptor) {
